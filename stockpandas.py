...
def percentchange(sym):

	symdf = df2[['Date',sym]]
	symmrv = symdf.iloc[-1:][sym]
	tendayavg = symdf[-10:].mean()[0]
	symchangetenday = (symmrv.max()-tendayavg)/tendayavg*100
	fiftydayavg = symdf[-50:].mean()[0]
	hundreddayavg = symdf[-100:].mean()[0]
	...
	if symchangetenday < 0:
		print sym, "\n adjusted close yesterday dropped",symchange, \
		"% compared to the average adjusted close over the past 10 days"
	if symchangetenday > 0:
		print sym, "\n adjusted close yesterday rose",symchange, \
		"% compared to the average adjusted close over the past 10 days"
		
	""" volume data pull and volume percent change """
	#print 'the rate of change is $',roc, 'today'
	print sym,'10 day average is $',tendayavg
	print sym,'50 day average is $',fiftydayavg
	print sym,'100 day average is $',hundreddayavg
	voldf = df5[['Date',sym]]
